...
大纲即目录：H2 章 · H3 度量项 · H4 度量子项。链接按标题 slug；稳定 ID 也写在标题里，可用 `@docs/next.md` 检索。

- [S1 系统属性](#s1-系统属性)（10 项 / 18 子项）— 代际、配比、类型、互联等硬件规格与系统契约
  - [S1-D1 源码跨代可移植性](#s1-d1-源码跨代可移植性)
    - [`S1-D1-eta_port`](#s1-d1-eta_port-跨代零改动编译通过率)
    - [`S1-D1-eta_perf`](#s1-d1-eta_perf-跨代性能达成率)
  - [S1-D2 API 破坏性变更（弱）](#s1-d2-api-破坏性变更弱) _弱_
    - [`S1-D2-n_brk`](#s1-d2-n_brk-每代-api-破坏性变更数)
    - [`S1-D2-eta_brk`](#s1-d2-eta_brk-破坏性变更可检测率)
  - [S1-D3 二进制兼容性（弱）](#s1-d3-二进制兼容性弱) _弱_
    - [`S1-D3-eta_load`](#s1-d3-eta_load-跨代可加载率)
  - [S1-D4 形态配比保持度](#s1-d4-形态配比保持度)
    - [`S1-D4-delta_ratio`](#s1-d4-delta_ratio-形态比变更幅度)
    - [`S1-D4-rho_rw`](#s1-d4-rho_rw-算子重编写比例)
  - [S1-D5 算力配比冲击度](#s1-d5-算力配比冲击度)
    - [`S1-D5-delta_pow`](#s1-d5-delta_pow-配比变化幅度)
    - [`S1-D5-rho_retune`](#s1-d5-rho_retune-tiling流水重调比例)
  - [S1-D6 配比变化重写量](#s1-d6-配比变化重写量)
    - [`S1-D6-L_rw`](#s1-d6-l_rw-配比变化重写代码行)
  - [S1-D7 核映射自适应度](#s1-d7-核映射自适应度)
    - [`S1-D7-eta_nmap`](#s1-d7-eta_nmap-自动核映射命中率)
  - [S1-A8 多版本一致性（补全）](#s1-a8-多版本一致性补全) _补全_
    - [`S1-A8-eta_ver`](#s1-a8-eta_ver-跨版本行为一致率)
    - [`S1-A8-n_drift`](#s1-a8-n_drift-跨版本语义漂移数)
  - [S1-B3 数值/类型支持度（红线）](#s1-b3-数值类型支持度红线) _红线_
    - [`S1-B3-h_bar`](#s1-b3-h_bar-类型转换平均跳数)
    - [`S1-B3-eta_type`](#s1-b3-eta_type-类型完备度)
  - [S1-E1 通算 overlap 线性可叠加度](#s1-e1-通算-overlap-线性可叠加度)
    - [`S1-E1-eta_ov`](#s1-e1-eta_ov-overlap-场景占比)
    - [`S1-E1-eta_lin`](#s1-e1-eta_lin-叠加效率)
    - [`S1-E1-L_blk`](#s1-e1-l_blk-手工同步阻塞代码行)

- [S2 编译友好](#s2-编译友好)（6 项 / 6 子项）— 编译器/工具链能自动吸收多少微架构暴露
  - [S2-A7 编译友好屏蔽率](#s2-a7-编译友好屏蔽率)
    - [`S2-A7-eta_sh`](#s2-a7-eta_sh-编译友好屏蔽率)
  - [S2-A13 自动同步消除率（补全）](#s2-a13-自动同步消除率补全) _补全_
    - [`S2-A13-eta_sync`](#s2-a13-eta_sync-自动同步消除率)
  - [S2-A14 自动 tiling 命中率（补全）](#s2-a14-自动-tiling-命中率补全) _补全_
    - [`S2-A14-eta_til`](#s2-a14-eta_til-自动-tiling-命中率)
  - [S2-A15 自动格式转换覆盖率（补全）](#s2-a15-自动格式转换覆盖率补全) _补全_
    - [`S2-A15-eta_fmt`](#s2-a15-eta_fmt-自动格式转换覆盖率)
  - [S2-A12 高层 API 替代覆盖率（弱 · 补全）](#s2-a12-高层-api-替代覆盖率弱-补全) _弱 · 补全_
    - [`S2-A12-eta_hl`](#s2-a12-eta_hl-高层-api-覆盖率)
  - [S2-A16 用户实际感知暴露度（弱 · 补全）](#s2-a16-用户实际感知暴露度弱-补全) _弱 · 补全_
    - [`S2-A16-n_perc`](#s2-a16-n_perc-用户须感知暴露项数)

- [S3 内存模型](#s3-内存模型)（4 项 / 6 子项）— 层级、片上资源、一致性、初始化
  - [S3-A1 内存层级暴露](#s3-a1-内存层级暴露)
    - [`S3-A1-deltaK`](#s3-a1-deltak-tiling-key-增长量)
    - [`S3-A1-r_mv`](#s3-a1-r_mv-搬运代码行占比)
  - [S3-A5 资源/约束暴露](#s3-a5-资源约束暴露)
    - [`S3-A5-L_mm`](#s3-a5-l_mm-内存管理代码行)
    - [`S3-A5-L_id`](#s3-a5-l_id-id-管理代码行)
  - [S3-A6 硬件约束暴露（DCCI）](#s3-a6-硬件约束暴露dcci)
    - [`S3-A6-L_dcci`](#s3-a6-l_dcci-dcci-代码行)
  - [S3-A10 设备初始化（补全）](#s3-a10-设备初始化补全) _补全_
    - [`S3-A10-L_init`](#s3-a10-l_init-设备初始化代码行)

- [S4 计算模型](#s4-计算模型)（4 项 / 8 子项）— 引擎、指令粒度、数值行为、API–指令耦合
  - [S4-B2 计算复杂度](#s4-b2-计算复杂度)
    - [`S4-B2-rho_ins`](#s4-b2-rho_ins-指令粒度比)
  - [S4-B7 计算行为表达（补全）](#s4-b7-计算行为表达补全) _补全_
    - [`S4-B7-n_beh`](#s4-b7-n_beh-显式饱和舍入特殊函数调用数)
  - [S4-A2 多流水暴露](#s4-a2-多流水暴露)
    - [`S4-A2-r_sw`](#s4-a2-r_sw-pipeline-切换手动同步代码占比)
    - [`S4-A2-eta_auto`](#s4-a2-eta_auto-自动同步覆盖率)
    - [`S4-A2-n_VF`](#s4-a2-n_vf-vf-表达数)
  - [S4-A4 API-指令耦合](#s4-a4-api-指令耦合)
    - [`S4-A4-p_bar`](#s4-a4-p_bar-入参平均数)
    - [`S4-A4-n_reg`](#s4-a4-n_reg-寄存器配置数)
    - [`S4-A4-delta_sig`](#s4-a4-delta_sig-跨版本签名差异数)

- [S5 控制模型](#s5-控制模型)（3 项 / 7 子项）— 同步、流水序域、ID 协议
  - [S5-A3 同步暴露](#s5-a3-同步暴露)
    - [`S5-A3-n_intra`](#s5-a3-n_intra-核内同步调用数)
    - [`S5-A3-n_inter`](#s5-a3-n_inter-核间同步调用数)
    - [`S5-A3-eta_xcore`](#s5-a3-eta_xcore-自动核间同步覆盖率)
  - [S5-A9 同步流水复杂度（补全）](#s5-a9-同步流水复杂度补全) _补全_
    - [`S5-A9-n_pipe`](#s5-a9-n_pipe-流水级数)
    - [`S5-A9-n_ord`](#s5-a9-n_ord-序域个数)
  - [S5-A11 ID 残留对消（补全）](#s5-a11-id-残留对消补全) _补全_
    - [`S5-A11-L_cancel`](#s5-a11-l_cancel-id-对消代码行)
    - [`S5-A11-n_resid`](#s5-a11-n_resid-对消后残留次数)

- [S6 访存易用性](#s6-访存易用性)（4 项 / 5 子项）— tiling、地址、对齐、随路搬运
  - [S6-B1 搬运复杂度（tiling）](#s6-b1-搬运复杂度tiling)
    - [`S6-B1-n_til`](#s6-b1-n_til-tiling-参数个数)
  - [S6-B4 地址计算（补全）](#s6-b4-地址计算补全) _补全_
    - [`S6-B4-L_addr`](#s6-b4-l_addr-地址计算代码行)
  - [S6-B5 地址对齐（补全）](#s6-b5-地址对齐补全) _补全_
    - [`S6-B5-n_pad`](#s6-b5-n_pad-对齐边界分支数)
  - [S6-B6 随路搬运（补全）](#s6-b6-随路搬运补全) _补全_
    - [`S6-B6-n_fix`](#s6-b6-n_fix-随路互斥拆分步数)
    - [`S6-B6-n_fmt`](#s6-b6-n_fmt-格式转换调用数)

- [S7 调试调优](#s7-调试调优)（5 项 / 16 子项）— 可观测、可重放、可隔离、可归因
  - [S7-C1 性能度量覆盖](#s7-c1-性能度量覆盖)
    - [`S7-C1-eta_evt`](#s7-c1-eta_evt-关键性能事件覆盖率)
    - [`S7-C1-eps_met`](#s7-c1-eps_met-指标相对误差)
    - [`S7-C1-rho_loss`](#s7-c1-rho_loss-数据丢失率)
    - [`S7-C1-eta_stall`](#s7-c1-eta_stall-stall-原因解释覆盖率)
  - [S7-C2 确定性与重放（红线）](#s7-c2-确定性与重放红线) _红线_
    - [`S7-C2-eta_det`](#s7-c2-eta_det-同输入结果一致率)
    - [`S7-C2-eta_rep`](#s7-c2-eta_rep-执行顺序可重放率)
  - [S7-C3 故障隔离与恢复（红线）](#s7-c3-故障隔离与恢复红线) _红线_
    - [`S7-C3-g_rst`](#s7-c3-g_rst-最小复位范围)
    - [`S7-C3-eta_rst`](#s7-c3-eta_rst-局部复位成功率)
    - [`S7-C3-MTTR`](#s7-c3-mttr-平均恢复时间)
  - [S7-C4 错误遥测（红线）](#s7-c4-错误遥测红线) _红线_
    - [`S7-C4-eta_ex`](#s7-c4-eta_ex-异常类型覆盖率)
    - [`S7-C4-eta_hit_fault`](#s7-c4-eta_hit_fault-异常检出率)
    - [`S7-C4-rho_sdc`](#s7-c4-rho_sdc-静默数据错误率)
    - [`S7-C4-eta_root`](#s7-c4-eta_root-首因识别率)
    - [`S7-C4-eta_snap`](#s7-c4-eta_snap-现场保存成功率)
  - [S7-C5 源码关联深度](#s7-c5-源码关联深度)
    - [`S7-C5-eta_map`](#s7-c5-eta_map-模型算子kernel指令映射完备度)
    - [`S7-C5-eta_hit`](#s7-c5-eta_hit-硬件事件可修改代码命中率)
## S0 总则
